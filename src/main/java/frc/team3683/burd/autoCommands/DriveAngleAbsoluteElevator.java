 ...
 */
public class DriveAngleAbsoluteElevator extends Command {
	...
	private double elevatorDegree;
	private double intakePos;
	private double intakeHeading;
	private double startingDegree;
	private double ticks;

	public DriveAngleAbsoluteElevator(double angle, double speed, double timeOut,
																		double elevatorPos, double elevatorDegree,
																		double intakePos, double intakeHeading) {
		// Use requires() here to declare subsystem dependencies
		this.speed = speed;
		this.angle = angle;
		this.timeOut = timeOut;
		this.elevatorPos = elevatorPos;
		this.elevatorDegree = elevatorDegree;
		this.intakePos = intakePos;
		this.intakeHeading = intakeHeading;
	...
	protected void initialize() {
		...
		startingDegree = Robot.getmDriveBase().getAbsoluteHeading();
		setTimeout(timeOut);
	...
	protected void execute() {
		...
		Robot.getmElevator().autoPID();
			...
			Robot.getmIntake().setWrist(intakePos);


		if(((Math.abs(Robot.getmDriveBase().getAbsoluteHeading() - targetAngle) < 3.0) && (Math.abs(Robot.getmDriveBase().getSpeedLeft()) < 2.0)) && (Math.abs(Robot.getmElevator().getHeight() - elevatorPos)) < 5){
			ticks++;
		} else {
			ticks = 0;
		}
